# Formatter [![Build Status](https://travis-ci.org/collab-corp/formatter.svg?branch=dev)](https://travis-ci.org/collab-corp/formatter)
...
In addition to calling get, it is also possible to get the results by casting the formatter to a string
`(string)Formatter::create('something')->{method}();`
...
<strong>Note:</strong> If the request value is a single dimensional array, the formatter method will be applied to every value on the array element. More complex arrays not tested/supported.
